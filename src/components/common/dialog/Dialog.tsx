...
const Dialog = (props: ComponentProps) => {
	...
	const actions = [];
	if (onCancel) actions.push(<Button id="launchmetricsDialogCancelBtn" className="btn--grey btn--no-background" flat onClick={onCancel}>{onCancelText}</Button>);
	if (onAccept) actions.push(<Button id="launchmetricsDialogAcceptBtn" className="btn--green btn--no-background" flat onClick={onAccept}>{onAcceptText}</Button>);

	const dialogHeader = (
		<>
			<div className="dialog-close">
				{onCancel ? <Button id="launchMetricsDialogClosBtn" className="btn--no-background" flat onClick={onCancel}><FontIcon>clear</FontIcon></Button> : null}
			</div>

...
const __getColumnWidth = <Data extends Record<string, unknown>>(
	rows: Data[],
	accessor: Column<Data>['accessor'],
	headerText: string,
...
const __generateFinalColumnsData = <Data extends Record<string, unknown>>(
	columns: Column<Data>[],
	data: Data[]
): Column<Data>[] => {
	return columns.map((column: any) => ({
		...column,
		columns: column.columns ? __generateFinalColumnsData(column.columns, data) : undefined,
		Cell: column.Cell ?? TableCellDefault,
		Header:
			typeof column.Header === 'string'
				? (props: any) => <TableHeaderCell {...props}>{column.Header}</TableHeaderCell>
				: column.Header,
		width:
			column.width ??
			(typeof column.Header === 'string'
				? __getColumnWidth(data, column.accessor, column.Header, column.getCellWidthText)
				: undefined)
	}));
};

const __generateFinalInitialState = <Data extends Record<string, unknown>>(
...
const __generateFinalSortTypes = <Data extends Record<string, unknown>>(
...
const Table = <Data extends Record<string, unknown>>(props: TableProps<Data>) => {
	...
	const finalColumns = React.useMemo(() => __generateFinalColumnsData(columns, data), [columns, data]);

	const finalInitialState = React.useMemo(
		() => __generateFinalInitialState(tableOptions, filters, globalFilter, pageIndex, pageSize),
		[filters, globalFilter, pageIndex, pageSize, tableOptions]
	);

	const finalSortTypes = React.useMemo(() => __generateFinalSortTypes(tableOptions.sortTypes), [
	...
	useUpdateEffect(() => {
		debouncedOnSettingsChanged({ pageIndex: state.pageIndex, pageSize: state.pageSize, sortBy: state.sortBy });

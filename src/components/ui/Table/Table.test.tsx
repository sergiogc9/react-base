...
import { range } from 'lib/imports/lodash';
...
const renderComponent = (props: Partial<TableProps<TestData>> = {}) => {
	return TestUtils.renderWithMockedStore(getComponent(props));
};
...
describe('Table', () => {
	...
	it('should render header with custom JSX', () => {
		renderComponent({
			columns: [
				defaultColumns[0],
				{ accessor: 'name', Header: props => <Table.HeaderCell {...props}>Awesome name</Table.HeaderCell> }
			]
		});
	...
	it('should filter rows with filters provided in initialState', () => {
		renderComponent({ tableOptions: { initialState: { filters: [{ id: 'name', value: defaultData[0].name }] } } });
	...
	it('should filter rows after updating prop', () => {
		const { rerender } = render(getComponent(), { wrapper: TestUtils.getWrapperWithMockedStore() });
	...
	it('should disable filter rows after updating prop', () => {
		const { rerender } = render(getComponent({ filters: [{ id: 'name', value: defaultData[0].name }] }), {
			wrapper: TestUtils.getWrapperWithMockedStore()
		});
	...
	it('should not filter rows if manualFilters is set', () => {
		renderComponent({
			tableOptions: { initialState: { filters: [{ id: 'name', value: defaultData[0].name }] }, manualFilters: true }
		});
	...
	it('should global filter rows with filters provided in initialState', () => {
		renderComponent({ tableOptions: { initialState: { globalFilter: defaultData[0].name } } });
	...
	it('should order rows by order provided in initialState', () => {
		renderComponent({ tableOptions: { initialState: { sortBy: [{ id: 'name' }] } } });
	...
	it('should toggle order when clicking column header', () => {
		...
		let firstCell = screen.queryAllByRole('cell')[0];
		expect(firstCell.querySelector('span')?.innerHTML).toBe('0');
	...
	it('should toggle order between all states when clicking column header', () => {
		renderComponent({ tableOptions: { initialState: { sortBy: [{ id: 'id' }] } } });
	...
	it('should toggle boolean order between all states when clicking column header', () => {
		...
		let firstRowCell = screen.queryAllByRole('cell')[2];
	...
	it('should call onSort change when disabling sorting', () => {
		renderComponent({ tableOptions: { initialState: { sortBy: [{ id: 'name', desc: true }] } } });
	...
	it('should paginate to default index after removing prop', () => {
		const { rerender } = render(getComponent({ pageIndex: 2 }), { wrapper: TestUtils.getWrapperWithMockedStore() });
	...
	it('should paginate with page size after removing prop', () => {
		const { rerender } = render(getComponent({ pageSize: 20 }), { wrapper: TestUtils.getWrapperWithMockedStore() });
	...
	it('should render column width based on getCellWidthText column option', () => {
		renderComponent({ columns: [{ ...defaultColumns[0], getCellWidthText: () => 'test' }, defaultColumns[1]] });
	...
	it('should render column width based on width column option', () => {
		renderComponent({ columns: [{ ...defaultColumns[0], width: 110 }, defaultColumns[1]] });
	...
	it('should render column width based on a function accessor', () => {
		renderComponent({
			columns: [{ ...defaultColumns[0], accessor: ((row: TestData) => row.id) as any, id: 'id' }, defaultColumns[1]]
		});

...
describe('Book entity', () => {
	...
	it('should update many books', () => {
		store.dispatch(
			actions.updateMany([
				{ id: book.id, changes: { title: 'New title' } },
				{ id: book3.id, changes: { title: 'New title 3' } }
			])
		);
		expect(selectors.selectTotal(store.getState())).toEqual(2);
		expect(selectors.selectAll(store.getState())).toEqual([
			{ ...book, title: 'New title' },
			{ ...book3, title: 'New title 3' }
		]);
	});
	...
	it('should upsert one book', () => {
		...
		expect(selectors.selectTotal(store.getState())).toEqual(2);
		expect(selectors.selectById(store.getState(), book.id)).toEqual({
			...book,
			title: 'New title 2'
		});
	});
	...
	it('should fetch books', async () => {
		await expectSaga(sagas)
			...
			.put(
				actions.fetchBooksError({
					code: 'FAKE_CODE',
					message: 'A fake API error has been thrown! 😄'
				})
			)

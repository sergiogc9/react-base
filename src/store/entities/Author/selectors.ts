...
const selectWithBooksById = createSelector(
	...
	(books, author) => author ? { ...author, books: author.books.map(bookId => books[bookId]) } as AuthorPopulated<"books"> : undefined
);
const selectAllWithBooks = createSelector(
	entitySelectors.selectAll,
	BookSelectors.selectEntities,
	(authors, books) => authors.map(author => ({ ...author, books: author.books.map(bookId => books[bookId]) } as AuthorPopulated<"books">))
);
const selectors = {
	...entitySelectors,
	selectWithBooksById,

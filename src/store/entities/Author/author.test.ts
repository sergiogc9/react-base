...
describe('Author entity', () => {
	...
	it('should update many authors', () => {
		store.dispatch(
			actions.updateMany([
				{ id: author.id, changes: { name: 'New name' } },
				{ id: author3.id, changes: { name: 'New name 3' } }
			])
		);
		expect(selectors.selectTotal(store.getState())).toEqual(2);
		expect(selectors.selectAll(store.getState())).toEqual([
			{ ...author, name: 'New name' },
			{ ...author3, name: 'New name 3' }
		]);
	});
	...
	it('should upsert one author', () => {
		...
		expect(selectors.selectTotal(store.getState())).toEqual(2);
		expect(selectors.selectById(store.getState(), author.id)).toEqual({
			...author,
			name: 'New name 2'
		});
	});
	...
	it('should select authors entities', () => {
		expect(selectors.selectEntities(store.getState())).toEqual({
			[author.id]: author,
			[author3.id]: author3
		});
	});
	...
	it('should select author with books by id', () => {
		...
		expect(selectors.selectWithBooksById(store.getState(), author.id)).toEqual({
			...author,
			books: [book]
		});
	});
